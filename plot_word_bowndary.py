...
def readCtm( ctm ) :
    ...
    for row in rows :
        ...
        if utt_id not in ctmDict :
            ctmDict[ utt_id ] = []
        ctmDict[ utt_id ].append( { 'phone_id': phone_id, 'start_time': start_time, 'end_time': end_time, 'phone_dur': phone_dur } )
    f.close()
    # print( ctmDict )
    return ctmDict
...
def plotWav( utt_id , cmtList , wavFile , saveFile ) :
    ...
    ax.set_title( 'Signal Wave... '+utt_id )
    ax.set_ylim(-fs, fs)
    ax.plot(Time,signal)
...
def wavPltAddRectangle( ax , cmtList , fs=16000 ) :
    for cmt in cmtList :
        ...
        rect = patches.Rectangle( (startTime,-fs+1) , phoneDur , fs*2-1 , linewidth=1 , edgecolor=colour , facecolor='none' , zorder=99) # (x,y), width, height
        # Add the patch to the Axes
        ax.add_patch(rect)
        ax.text(startTime+phoneDur/2, -fs+20, text, color='k', fontweight='bold')
    return ax 
...
if __name__ == "__main__" :  
    ...
    # drow graph
    plotWav( 'md011010' , ctmDict['md011010'] , wavDict['md011010'] , saveFile )
